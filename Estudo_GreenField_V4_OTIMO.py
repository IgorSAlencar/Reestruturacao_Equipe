...
def reconcile_district_geometries(
    ...
    areas = target.geometry.area.to_numpy(float) / 1_000_000
    representative = gpd.GeoSeries(
        target.geometry.representative_point(), crs="EPSG:5880"
    ).to_crs(epsg=4326)
    target["AREA_KM2_GEOMETRIA"] = areas
...
def assign_stores_to_v4_demand(
    stores: pd.DataFrame,
    demand: pd.DataFrame,
    split: set[str],
    district_geo: gpd.GeoDataFrame | None,
    ...
    for code in sorted(split):
        ...
        assigned["METODO_ASSOCIACAO_UNIDADE"] = "PONTO_DENTRO_DISTRITO_RECONCILIADO"
        missing = city_stores[
            ~city_stores["CHAVE_LOJA"].isin(set(assigned["CHAVE_LOJA"]))
        ].copy()
        fallback = v3.assign_split_stores_by_nearest_district(missing, city_units)
    ...
    logging.info("Lojas associadas na V4: %s", len(out))
    return out
...
def prepare_v4_demand(
    ...
    stores_by_unit = assign_stores_to_v4_demand(stores, demand, split, district_geo)
    store_count = (
...
def main(argv: list[str] | None = None) -> None:
    ...
    try:
        ...
        district_reference, district_geo, district_geometry_audit = (
            reconcile_district_geometries(
                district_reference,
                district_geo,
                municipal_geo,
                metropolitan_codes,
        ...
        district_audit = pd.concat(
            [district_geometry_audit, district_audit], ignore_index=True, sort=False
        )
