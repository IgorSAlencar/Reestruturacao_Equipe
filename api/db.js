...
export function listDrafts() {
  return db.prepare('SELECT id,name,base_scenario_id,revision,created_at,updated_at FROM drafts ORDER BY updated_at DESC').all().map(r=>({
...
export function getDraft(id) {
  const r=db.prepare('SELECT * FROM drafts WHERE id=?').get(id); if(!r)return null;
...
export function createDraft(id,name,baseScenarioId,data) {
...
export function updateDraft(id,name,revision,data) {
...
export function deleteDraft(id){ return db.prepare('DELETE FROM drafts WHERE id=?').run(id).changes>0; }
