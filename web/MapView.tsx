...
export function areaColor(area:string,areas:string[]=[]){
  ...
  return COLORS[Math.abs([...area].reduce((n,c)=>(n*31+c.charCodeAt(0))|0,0))%COLORS.length];
}
...
export default function MapView(p:Props){
  ...
  const areaNames=useMemo(()=>[...new Set(p.data?.poles.map(pole=>pole.area)||[])].sort((a,b)=>a.localeCompare(b,'pt-BR')),[p.data]);
  const colorFor=(area:string)=>areaColor(area,areaNames);
  ...
  useEffect(()=>{
    ...
    m.on('load',()=>{
      ...
      m.on('click','municipality-fill',(e)=>{const code=String(e.features?.[0]?.properties?.id||e.features?.[0]?.properties?.CD_MUN||'');const units=dataRef.current?.units.filter(x=>x.municipalityCode===code)||[];const unit=units.find(x=>x.poleId===callbacks.current.selectedPole)||units[0];if(unit)callbacks.current.onUnit(unit.id,!!e.originalEvent.ctrlKey||!!e.originalEvent.metaKey);});
    ...
    return()=>{setStyleReady(false);m.remove();map.current=null;};
  },[p.token,p.styleUrl]);
  ...
  useEffect(()=>{
    ...
    p.data.poles.filter(pole=>!p.selectedArea||pole.area===p.selectedArea).forEach(pole=>{
      ...
      el.style.setProperty('--pole-color',colorFor(pole.area));
    ...
    const features=base.features.map((feature:any,index)=>{
      ...
      return {...feature,properties:{...properties,_unitId:unit?.id||id,_poleId:unit?.poleId,_color:colorFor(pole?.area||'SEM ÁREA'),_visible:p.showAll||unit?.poleId===p.selectedPole,_selected:unit?p.selectedUnits.has(unit.id):false}};
    });
    ...
    p.data.units.forEach(unit=>{
      const pole=p.data!.poles.find(x=>x.id===unit.poleId);
      colors.set(unit.municipalityCode,colorFor(pole?.area||'SEM ÁREA'));
    ...
    m.setPaintProperty('municipality-fill','fill-opacity',p.showPopulation?['case',['has','id'],.76,.76] as any:.1);
  ...
  },[p.data,p.selectedPole,p.selectedUnits,p.selectedArea,p.showAll,p.showPopulation,p.population,p.editable,styleReady,areaNames]);
